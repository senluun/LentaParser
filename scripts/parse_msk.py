"""
Скрипт для выполнения запросов к API Ленты для получения товаров Москвы.

Этот модуль предоставляет функцию make_lenta_request для выполнения
POST запросов к API каталога Ленты с заданными параметрами.
Результат сохраняется в файл msk_results.json.
...
def make_lenta_request(payload):
    """
    Выполняет POST запрос к API каталога Ленты.
    
    Args:
        payload (dict): Тело запроса с параметрами фильтрации, сортировки и пагинации
    
    Returns:
        dict or None: JSON ответ от API или None в случае ошибки
    """
    # URL эндпоинта API для получения товаров
    ...
    headers = {
        ...
        "Host": "api.lenta.com",
        "LocalTime": "2026-02-19T21:55:53.405684Z",
        "Qrator-Token": "2f3b92be0ccf30f53024be6ea835c1bb",
        "SessionToken": "CC5DD8D5B6AF243E80E5254B8847D9D0",
        "Timestamp": "1771538153",
...
if __name__ == "__main__":
    ...
    example_payload = {
        "categoryId": 4,           # ID категории товаров
        "filters": {
            "multicheckbox": [],   # Фильтры не используются
            "checkbox": [],
            "range": []
        },
        "sort": {
            "type": "popular",     # Сортировка по популярности
            "order": "desc"
        },
        "limit": 100,              # Количество товаров
        "offset": 0                # Начинаем с первого
    }
    
    # Выполняем запрос
    result = make_lenta_request(example_payload)
    ...
    if result:
        ...
        output_file = DATA_DIR / "msk_results.json"
