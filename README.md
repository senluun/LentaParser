...
### parse_multiple_cities.py (дополнительно)
Расширенная версия для парсинга множества городов.
- Поддержка 15+ городов России
- Обработка ошибок и повторные попытки
- Промежуточное сохранение результатов (checkpoints)
- Детальная статистика по каждому городу
- Результат: `data/items_multiple_cities_TIMESTAMP.json`

### analyze_price_differences.py (дополнительно)
Анализирует различия в ценах между городами.
- Сравнивает цены на одинаковые товары
- Находит товары с разными ценами
- Выявляет промо-акции только в одном городе
- Результат: консольный отчет + `data/price_analysis_report.json`
...
- API токены могут устаревать - обновите заголовки при необходимости

## Анализ цен

**Результат анализа СПб vs Москва:**
- ✅ Все 100 товаров присутствуют в обоих городах
- ✅ Цены **полностью идентичны** (100% совпадение)
- ✅ Промо-акции одинаковые в обоих городах

**Вывод:** Лента использует единую ценовую политику для Москвы и Санкт-Петербурга.

Для анализа цен запустите:
```bash
python scripts/analyze_price_differences.py
```
